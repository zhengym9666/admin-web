...
(function(obj){
   ...
   var MemberInfo={
       ...
       addMemberInfo:function(){
    	   ...
    	   $("#edit-paramDesc").val('');
    	   $("#edit-CodeId").val('');
    	   /*$("#edit-paramState li.choosed").removeClass();
    	   $("#edit-paramState li").eq(0).addClass("choosed");
    	   $("#edit-paramState").prev("button").children("p").text($(".choosed").text());*/
    	   $(".right-alert_content ul .TextBtn-import").show();
       },
       
       querySysParamInfos:function(){
           $.ajax({
...
//                               +'<td state='+item.STATE+'>'+(item.STATE=='U'?'启动':'关闭')+'</td>'
           ...
           $.ajax({
               ...
               success : function(response) {
                   if(response.resultFlag){
                       var obj = {};
                       obj["Ptext"] = "操作成功";
                       obj['func']=MemberInfo.querySysParamInfos();
       ...
       deleteSysParamInfo:function(paramType,paramName){
           var operateWarnObj = {
               Ptext: "您确定要删除吗？",
               Stext: "一经删除可不恢复",
               func:function(){
                   $.ajax({
                       url : SmsSysParamInfo.actionPath + "delete&",
                       type:'post',
                       async:false,
                       cache:false,
                       data : {
                       ...
                       success : function(response) {
                           if(response.resultFlag){
                               ...
                               obj["Ptext"] = "操作成功";
                               operationTipsTrue(obj);
                               obj['func']=SmsSysParamInfo.querySysParamInfos();
                           }else{
                               var obj = {};
                               obj["Ptext"] = response.Msg;
                               operationTipsFailed(obj);
