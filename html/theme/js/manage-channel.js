//初始化运营位反馈状态列表
initOprRespStateList(true)
initOprRespStateList(false)
/* 分页 */
...
$('.form-right').on('blur','#selectMenu li',function() {
	$("#ownContacts ul").html("");
	$("#ownContacts p").text("请选择");
	var id=$(this).children('input').val();
	var contactActionPath = rootPath + "/business/com.airmdp.web.execute.channeloprpos.action.ContactListAction?action=";
		$.ajax({
			url : contactActionPath + "queryContact&",
			type : 'post',
			async: false,
			cache: false,
			data : {channelId : id},
			success : function(response) {
				...
				$.each(resp.root, function(i, item) {
					var str="<li>"+item.CONTACT_NAME+"<input type='hidden' value='"+item.CONTACT_CODE+"'/></li>";
		    		$("#ownContacts ul").append(str);
				});
			},
...
$('.form-btns .before').on('click',function(){
	var JQ_parent =$(this).parents('.form-input-step');
    var JQ_obj =  $(this).parents('.myright-alert').find('.right-alert-tabs_menu li.current');
...
function initOprRespStateList(readOnly){
    $.ajax({
        url : rootPath + "/business/com.airmdp.web.execute.channeloprpos.action.OprPosListAction?action=queryOprRespStateList",
        type : 'post',
        async: false,
        cache: false,
        success : function(response) {
            var resp = eval('(' + response + ')');
            var selector = readOnly ? "#oprRespStateView" : "#oprRespState";
            $(selector + " .oprRespStateList").html("");
            $.each(resp.root, function(i, item){
                var div = '<div class="checkbox-item">'+
                            '<div class="checked-box ' + (readOnly ? 'disabled' : '') + '"><i class="iconfont"></i></div>'+
                            '<span>' + item.name + '<input type="hidden" value="' + item.id + '"></span>'+
                        '</div>';
                $(selector + " .oprRespStateList").append(div);
            });
        },
        error : function(response) {
            error();
